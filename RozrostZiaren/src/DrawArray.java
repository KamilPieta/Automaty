			...
			for (int j = 0; j < arr.length; ++j) {
				...
				if (arr[i][j] == 0) {
					...
					for (Integer key : koloryMapa.keySet()) {
						if (key == arr[i][j]) {
					//		System.out.println("Key found! " + key + " is equal " + arr[i][j]);

					//		System.out.println("choosen color for key "+arr[i][j]+" are " + Arrays.toString(koloryMapa.get(arr[i][j])));
					...
					if (setColor==false) {
						...
						kolors[1] = (Integer) G;
						kolors[2] = (Integer) B;
					//	System.out.println("adding to key "+arr[i][j] +" values "+Arrays.toString(kolors));
